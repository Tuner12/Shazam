    ...
    for i in range(epoch):
        ...
        for item in topk_data:
            ...
            for ans_entry in answers:
                if isinstance(ans_entry, dict):
                    ans_text = ans_entry.get('answer', '')
                    prob = ans_entry.get('prob', None)
                else:
                    ans_text = ans_entry
                    prob = None
                normalized.append({
                    'answer': pre_answer(ans_text),
                    'prob': prob
                })
            qid_to_topk[qid] = normalized
        ...
        def compute_at_k(k: int):
            ...
            for qid in vqa.getQuesIds():
                ...
                totals[t] += 1
                preds_at_k = preds[:k]
                pred_texts = [entry['answer'] for entry in preds_at_k]
                if gt in pred_texts:
                    hits['overall'] += 1
                    hits[t] += 1
            return {
