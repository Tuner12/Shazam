...
# ------------------ 数据加载 ------------------
train_features1, train_labels = torch.load("/ailab/public/pjlab-smarthealth03/leiwenhui/YushengTan/Combined_Pathology_Models/features4HunCRC/virchow_train_features.pt", weights_only=True)
train_features2, train_labels = torch.load("/ailab/public/pjlab-smarthealth03/leiwenhui/YushengTan/Combined_Pathology_Models/features4HunCRC/uni_v1_train_features.pt", weights_only=True)
train_features3, train_labels = torch.load("/ailab/public/pjlab-smarthealth03/leiwenhui/YushengTan/Combined_Pathology_Models/features4HunCRC/phikon_v2_train_features.pt", weights_only=True)
train_features4, train_labels = torch.load("/ailab/public/pjlab-smarthealth03/leiwenhui/YushengTan/Combined_Pathology_Models/features4HunCRC/gigapath_train_features.pt", weights_only=True)

test_features1, test_labels = torch.load("/ailab/public/pjlab-smarthealth03/leiwenhui/YushengTan/Combined_Pathology_Models/features4HunCRC/virchow_test_features.pt", weights_only=True)
test_features2, test_labels = torch.load("/ailab/public/pjlab-smarthealth03/leiwenhui/YushengTan/Combined_Pathology_Models/features4HunCRC/uni_v1_test_features.pt", weights_only=True)
test_features3, test_labels = torch.load("/ailab/public/pjlab-smarthealth03/leiwenhui/YushengTan/Combined_Pathology_Models/features4HunCRC/phikon_v2_test_features.pt", weights_only=True)
test_features4, test_labels = torch.load("/ailab/public/pjlab-smarthealth03/leiwenhui/YushengTan/Combined_Pathology_Models/features4HunCRC/gigapath_test_features.pt", weights_only=True)
...
class EarlyStopping:
    def __init__(self, patience=5, min_delta=1e-4, lambda_ba=0.8, lambda_f1=0.2):
...
# 初始化早停对象
early_stopping = EarlyStopping(patience=50, min_delta=1e-4, lambda_ba=0.8, lambda_f1=0.2)
...
class CrossAttentionClassifierWithDistillation(nn.Module):
    def __init__(self, input_dims, d_model=512, num_classes=6, num_layers=2):
...
def feature_distillation_loss(student_features, expert_features_list, feature_mappers):
    ...
    for expert_feature, mapper in zip(expert_features_list, feature_mappers):
        ...
        # print("expert",mapped_expert_feature.shape)
...
def calculate_balanced_accuracy(predictions, labels):
    predictions = torch.argmax(predictions, dim=1)
    return balanced_accuracy_score(labels.cpu().numpy(), predictions.cpu().numpy())

def calculate_weighted_f1(predictions, labels):
    """计算 Weighted F1 指标"""
    predictions = torch.argmax(predictions, dim=1)
...
for epoch in range(num_epochs):
    ...
    print(
        ...
        f"Val Top-1: {val_top1_acc:.4f}, Val BA: {val_ba:.4f}, Val F1: {val_f1:.4f}, "
